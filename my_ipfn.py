''' @author: Dirguis - I made some edits in order to output each result of iteration for graphing '''
#!/usr/bin/env python
...
class ipfn(object):
    ...
    def ipfn_df(self, df, aggregates, dimensions, weight_col='total'):
        ...
        #print(df.groupby('dwtypenx')['total'].sum())
        # store dfs to try and make scatter plots

        # xipppp = df.groupby('dwtypenx')['total'].sum()
        # xpjppp = df.groupby('hhsizex')['total'].sum()
        # xppkpp = df.groupby('hhtype7')['total'].sum()
        # xppplp = df.groupby('agehrp6x')['total'].sum()
        # xppppm = df.groupby('sexhrp')['total'].sum()
